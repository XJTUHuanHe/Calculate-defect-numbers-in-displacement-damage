...
def modify(frame, input, output):
    ...
    selection[:] = (site_type == 1) & (occupancies[:,0] == 0) & (occupancies[:,1]==0)   #Vacancy A
    
    # Additionally output the total number of antisites as a global attribute:
    output.attributes['A_vacancy_count'] = np.count_nonzero(selection)
...
export_file(node, "A_vacancy_numbers.txt", "txt", 
    columns = ['Timestep', 'A_vacancy_count'],
...
export_file(node, "A_vacancy_position.xyz", "xyz", 
